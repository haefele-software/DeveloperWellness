...
- Q: How should the app authenticate to the AI service without interactive sign-in at start-up? → A: With a plain API key supplied through configuration, calling the model deployment directly. The previously planned pre-created hosted agent is dropped, because its endpoint accepts only interactive-capable Entra ID identities; the application owns its AI instructions instead. No developer sign-in is ever required to start the application.
...
- **Provisioned AI capability**: The organisation already has a language-model deployment provisioned on its AI platform; the application consumes it through configuration (endpoint, API key, deployment name), in the same manner as the GitHub credential, and owns its own AI instructions. Start-up never requires interactive sign-in (2026-07-17 clarification). The specific platform and connection technology are planning decisions recorded in the plan (research R3).
