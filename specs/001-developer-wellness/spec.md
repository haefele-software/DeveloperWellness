...
- **SC-008**: When tone signals are available, a developer whose negative share exceeds the threshold shows a frustration mention on the check-in roster in the same load, and tone appears nowhere else in the application.
...
- **Tone surfaces only on the roster**: Tone is computed per comment author solely to feed the check-in roster's frustration mention. There is no tone page, no per-project tone stat, and no per-comment verdict or quoted excerpt anywhere; this keeps the signal supportive rather than disciplinary, per the 2026-07-17 clarification.
